...
internal fun flattenInline(
    ...
    fun walk(node: AstNode, style: Style) {
        when (node) {
            ...
            is AstNode.InlineCodeNode -> push(node.code, styles.inlineCode)
            is AstNode.UrlNode -> {
                ...
                push(label, styles.link)
            }
            is AstNode.SimpleUrlNode -> push(node.url, styles.link)
            is AstNode.ImageNode -> push("[image: ${node.urlUrlNode.urlNameNode.name.ifBlank { node.urlUrlNode.url.url }}]", styles.imageFallback)
